...
def start_traffic_light_recognition(camera_type):
    """
    启动红绿灯识别
    ...
    try:
        ...
        if camera_type == "dabai":
            ...
            cmd1 = "roslaunch realsense2_camera rs_camera.launch"
        else:
        ...
        # 启动红绿灯识别
        cmd2 = "roslaunch darknet_ros yolo_v3_tiny.launch"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python3 module13_traffic_light.py [start|stop] [type]")
    ...
    if action == "start":
        if len(sys.argv) < 3:
            print("启动红绿灯识别时必须指定相机类型")
            print("用法: python3 module13_traffic_light.py start [type]")
            print("type可选值: dabai(大白相机), realsense(Realsense相机)")
            sys.exit(1)
        
        camera_type = sys.argv[2]
        start_traffic_light_recognition(camera_type)
    elif action == "stop":
        stop_all()
    else:
        print(f"未知操作: {action}")
