...
def start_integrated_control():
    ...
    try:
        ...
        # 启动集成控制
        cmd = "roslaunch limo_py_drive limo_mode_controller.launch"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python3 module6_integrated_control.py [start|stop]")
        sys.exit(1)
        
    action = sys.argv[1]
    
    if action == "start":
        start_integrated_control()
    elif action == "stop":
        stop_all()
    else:
        print(f"未知操作: {action}")
